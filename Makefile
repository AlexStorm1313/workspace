...
push:
	make build
	podman login -u kubeadmin -p $(kluster oc whoami -t) $(kluster oc get route default-route -n openshift-image-registry --template='{{.spec.host}}') --tls-verify=false

	@# podman push localhost/${NAMESPACE}-pod-container:tag $(kluster oc get route default-route -n openshift-image-registry --template='{{.spec.host}}')/${NAMESPACE}-pod-container:tag --tls-verify=false

# Install the Helm Chart on the OKD/OpenShift cluster
install:
	podman run --privileged -it --rm -v ${KUBECONFIG_PATH}:/root/.kube/config -v ./:/apps -w /apps alpine/helm:latest --namespace=${OKD_NAMESPACE} install ${NAMESPACE} --values values.yaml --values ./values.okd.yaml .
	make push

# Upgrade the Helm Chart on the OKD/OpenShift cluster
upgrade:
	make push
	podman run --privileged -it --rm -v ${KUBECONFIG_PATH}:/root/.kube/config -v ./:/apps -w /apps alpine/helm:latest --namespace=${OKD_NAMESPACE} upgrade ${NAMESPACE} --values values.yaml --values ./values.okd.yaml .

# Uninstall the Helm Chart on the OKD/OpenShift cluster
uninstall:
	podman run --privileged -it --rm -v ${KUBECONFIG_PATH}:/root/.kube/config -v ./:/apps -w /apps alpine/helm:latest --namespace=${OKD_NAMESPACE} uninstall ${NAMESPACE}
